...
public class DownloadThread implements FcpListener, Runnable {
	...
	public void run() {
		...
		try {
			...
			download(today, Mode.ACTIVE);
			download(today, Mode.INACTIVE);
	...
	private void download(LocalDate date, Mode mode)
		...
		if (iteration == 1 && mode == Mode.ACTIVE &&
				Config.getInstance().getFastMessageCheckEnabled()) {
	...
	private void downloadRequests(String label, List<DownloadRequest> requests)
		...
		final int maxFcpRequests = Config.getInstance().getMaxFcpRequests();
	...
	private List<Integer> getEligibleIdentities(LocalDate date, Mode mode) {
		final int inactivityTimeout =
			Config.getInstance().getInactivityTimeout();
		...
		if (mode == Mode.ACTIVE) {
			...
			final int inactivityRetryInterval=
				Config.getInstance().getInactivityRetryInterval();
