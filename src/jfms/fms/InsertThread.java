...
import jfms.captcha.CaptchaGenerator;
...
public class InsertThread implements FcpListener, Runnable {
	...
	private void insertIntroductionPuzzle(int localIdentityId,
		...
		Store store = FmsManager.getInstance().getStore();
		int unsolvedCount = store.getUnsolvedPuzzleCount(localIdentityId, date);
		if (unsolvedCount > 0) {
			return;
		}

		CaptchaGenerator generator = new CaptchaGenerator();
		IntroductionPuzzle puzzle = generator.generatePuzzle();
		puzzle.setUUID(UUID.randomUUID(localIdentity.getSsk()));

		IntroductionPuzzleWriter xmlWriter = new IntroductionPuzzleWriter();
		byte[] xml = xmlWriter.writeXml(puzzle);
		if (xml.length == 0) {
			return;
		}

		int index = store.getNextIntroductionPuzzleIndex(localIdentityId, date);
		InsertRequest request = new InsertRequest(
				RequestType.INTRODUCTION_PUZZLE,
				localIdentityId, date, index);
		request.setClientData(puzzle);
		final String key = Identity.getIntroductionPuzzleKey(
				localIdentity.getPrivateSsk(), date, index);
