...
public class TrustManager {
	...
	public void initialize() {
		if (localTrustListTrust == null || localMessageTrust == null) {
			...
			int identityId = Config.getInstance().getDefaultId();
	 ...
	 * @param identityId store ID of identity
	...
	public void updateLocalTrust(int localIdentityId, int identityId,
		...
		if (localIdentityId == Config.getInstance().getDefaultId()) {
	...
	public Set<Integer> getMessageTrustedIds() {
		...
		boolean includeNullPeerTrust =
			Config.getInstance().getDownloadMsgListsWithoutTrust();
	...
	public boolean isTrustListTrusted(int identityId, int minPeerTrust) {
		...
		if (localTrust != null) {
			return localTrust >= Config.getInstance()
				.getMinLocalTrustListTrust();
		}
	...
	public boolean isMessageTrusted(int identityId,
		...
		if (localTrust != null) {
			return localTrust >= Config.getInstance()
				.getMinLocalMessageTrust();
		}

		Integer peerTrust = peerMessageTrust.get(identityId);
		if (peerTrust != null) {
			return peerTrust >= Config.getInstance()
				.getMinPeerMessageTrust();
		}
		...
		Map<Integer, Map<Integer, Integer>> trustByTargetId)
	{
		...
		final double alpha = (double)(config.getIndirectTrustWeight())/100.0;
		final boolean excludeNullTrust = config.getExcludeNullTrust();
	...
	private void calculateTrustListTrusts() {
		...
		LOG.log(Level.FINE,
				...
				new Object[]{localIdentityId, config.getIndirectTrustWeight()});
	...
	private void calculateMessageTrusts() {
		...
		final int minPeerTrustlistTrust = Config.getInstance()
			.getMinPeerTrustListTrust();
